...
This script first automatically analyzes multiple videos to find an approximate
...
offsets (as timestamps) to 'sync_offsets_final.csv' and displays a
side-by-side image of the synchronized frames for verification.
"""
import cv2
import numpy as np
import os
...
DISPLAY_HEIGHT = 480
# The script will only search for the flash within the first N seconds of the video.
SEARCH_DURATION_SECONDS = 10


# --- Helper Function for Manual Adjustment ---
...
def find_and_fine_tune_sync():
    ...
    video_paths = []
    while True:
        print("\nA file dialog will open. Please select a video file.")
    ...
    for data in initial_flash_data:
        ...
        adjusted_flash_data.append({
            ...
            'verification_image': final_frame_image # Store the actual image
        })

    # --- 4. Save New CSV with Timestamps ---
    output_filename = 'sync_offsets_final.csv'
